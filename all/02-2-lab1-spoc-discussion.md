...
```
1.实现一个可显示字符串的bootloader
    这一部分的功能就是告诉我bootloader被加载起来了
    
    实现步骤：
        由于BIOS里面都有可以显示字符串的程序，只要将这一部分程序拿过来就可以显示字符串。

2.实现可加载ELF格式文件的bootloader
    实现步骤：
      （1）初始化寄存器
      （2）建立GDT
      （3）使能段机制
...
由于uCore里调用系统调用都通过统一的接口调用，在那里增加一个计数器，可以存下每个系统调用的调用次数。
再增加一个系统调用，用于返回这个系统调用的调用次数信息。
