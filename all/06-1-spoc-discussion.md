...
实现：
https://github.com/Socyrus/os_exercises/blob/master/all/scheduler-homework.py


(２)扩展练习1:理解并实现MLFQ调度算法　可基于python, ruby, C, C++，LISP等）模拟实现，并给出测试，在试验报告写出设计思路和测试结果分析。

实现：
https://github.com/Socyrus/os_exercises/blob/master/all/mlfq.py

根据MLFQ算法，设置了四个优先级，每个优先级的时间片随着优先级下降而倍增
当任务进入队列时，先放进最高优先级的队列中
按照优先级高低，从最高优先级的队列中，找到第一个任务执行，若时间片结束时，任务未完成，则加入到下一优先级的队列中。
...
Execution trace:
  [ time   0 ] Run job   0 for 1.00 secs, priority:   0
  [ time   1 ] Run job   1 for 1.00 secs, priority:   0
  [ time   2 ] Run job   2 for 1.00 secs, priority:   0
  [ time   3 ] Run job   0 for 2.00 secs, priority:   1
  [ time   5 ] Run job   1 for 2.00 secs, priority:   1
  [ time   7 ] Run job   2 for 2.00 secs, priority:   1
  [ time   9 ] Run job   0 for 4.00 secs, priority:   2
  [ time  13 ] Run job   1 for 4.00 secs, priority:   2
  [ time  17 ] Run job   2 for 1.00 secs ( DONE at 18.00 ), priority:   2
  [ time  18 ] Run job   0 for 2.00 secs ( DONE at 20.00 ), priority:   3
  [ time  20 ] Run job   1 for 1.00 secs ( DONE at 21.00 ), priority:   3

Final statistics:
  Job   0 -- Response: 0.00  Turnaround 20.00  Wait 11.00
  Job   1 -- Response: 1.00  Turnaround 21.00  Wait 13.00
  Job   2 -- Response: 2.00  Turnaround 18.00  Wait 14.00

  Average -- Response: 1.00  Turnaround 19.67  Wait 12.67
```
